...
## [Release: Report Pack v1.10] - 2025-12-28

### Report Pack: единый XLSX-отчёт со всеми ключевыми таблицами

#### ✅ Релизные изменения (v1.10 final)

- **Report Pack (report_pack.xlsx)**: единый XLSX-отчёт, собирается из CSV файлов
- **Best-effort XLSX**: отсутствие CSV не ломает генерацию (листы пропускаются или содержат "missing")
- **Graceful fallback engine**: openpyxl → xlsxwriter → warning (CSV остаются)
- **Нормализация tz-aware datetime**: конвертация в tz-naive при экспорте в XLSX (Excel не поддерживает timezone-aware)
- **Stage A: XLSX экспорт в try/except**: CSV пайплайн не падает при ошибках XLSX
- **Исправление теста xlsx_data_integrity**: совместимость с разными версиями pandas (reset_index вместо check_index)
...
1. **CSV остаются**: XLSX дополняет, не заменяет CSV-выгрузки (source of truth)
